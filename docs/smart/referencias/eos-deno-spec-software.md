...
> **Estudo de referência curado pelo usuário.** Reproduzido em `referencias/` para a plataforma Smart. Ver mapa de absorção em [00-absorcao-e-ajustes.md](00-absorcao-e-ajustes.md).
...
*Sequência: herdar (medidor + DERMS + OSS) → alcançar (plataforma, com fork) → ocupar (CCEE/BR + agent-fabric). A decisão build/fork/buy por domínio é o que evita reconstruir o que o open-source já resolveu. Premissas regulatórias (CCEE/ACL/VPP BT) a validar. Projeto EOS-DENO · spec de software · passo 3.*
